...
from protocols.protocols import Game as Game_
from protocols.protocols import Pawn
from protocols.protocols import Player as Player_



class Player(CurrentPlayer, Player_):
...
class HumanPlayer(Player, CurrentPlayer, Player_):
    def get_input_row_col(self, pos):
        x, y = pos
        row = y // FIELD_SIZE
        col = x // FIELD_SIZE
    ...
    def select(self, row, col, game: Game_):
        if game.selected:
    ...
    def _move(self, row, col, game: Game_):
        pawn = game.board.get_pawn(row, col)
...
class BotPlayer(Player, CurrentPlayer, Player_):

    def min_max(self, board: Board, depth: Union[complex, float, int], maximize, game: Game_):
    ...
    def get_all_moves(self, board: Board, game):
        moves = []
    ...
    def simulate_move(self, pawn, move: Sequence, board: Board, skip):
    ...
    def draw_processing_board(self, game: Game_, pawn: Pawn):
